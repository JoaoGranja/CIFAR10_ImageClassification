...
class ImageIAATransform(ImageTransform, BasicIAATransform):
    pass
...
class Transpose(ImageTransform):
    def apply(self, img, **params):
        return F.transpose(img)


class RandomRotate90(ImageTransform):
...
class ShiftScaleRotate(ImageTransform):
...
class RandomBrightness(ImageTransform):
...
class RandomContrast(ImageTransform):
...
class GaussNoise(ImageTransform):
...
class InvertImg(ImageTransform):
    def apply(self, img, **params):
        return F.invert(img)


class ToThreeChannelGray(ImageTransform):
...
class RandomLine(ImageTransform):
    def apply(self, img, **params):
        return F.random_polosa(img)


class AddChannel(ImageTransform):
    def __init__(self, prob=1.):
        super().__init__(prob)
