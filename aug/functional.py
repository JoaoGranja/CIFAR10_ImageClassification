...
def transform_image(image_data):
    ...
    '''
    aug_image_data = np.zeros(image_data.shape, dtype='uint8')
    
    for i in range(image_data.shape[0]):
        # Image translation    
        aug_image = translate_image(image_data[i])

        # Image Resize    
        aug_image = resize_image(aug_image)

        # Image Rotation    
        aug_image_data[i] = rotate_image(aug_image)

    return aug_image_data
...
def normalize_image(image_data):
    ...
    :return: Normalized image data
    """
    norm_image_data = np.subtract(image_data.astype(int),128)/128
